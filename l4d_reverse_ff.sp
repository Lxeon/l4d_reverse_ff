...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
    ...
    // fix if in god mode(readyup), Attacking teammates will cause them to heal.
    bool isGod = FindConVar("god").BoolValue;
    if (isGod)
    {
        return Plugin_Continue;
    }
    ...
    if (IsValidClientAndInGameAndSurvivor(attacker) && IsValidClientAndInGameAndSurvivor(victim) && victim != attacker)
    {
        ...
        // To determine in advance whether someone is incapped,
        // avoid being unable to properly reverse damage after being incapped by a single shot.
        bool isVictimIncapped = ReverseIfVictimIncapped(victim);

        // Calculate the distance in advance to avoid losing the ability to reverse damage after someone dies,
        // thus preventing them from gaining the necessary distance information.
        ...
        if (weapon > 0 || bWeaponGL || bWeaponMG)
        {
            ...
            if (!(ReverseIfAttackerAdmin(attacker) || ReverseIfVictimBot(victim) || isVictimIncapped || ReverseIfAttackerIncapped(attacker) || ReverseIfMountedgun || ReverseIfMelee || ReverseIfChainsaw) && (!g_bGrace[victim]))
            {
                // special ammo checks
